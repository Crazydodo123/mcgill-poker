...
class Table():
    ...
    def remove_player(self, player):
        self.players = [p for p in self.players if p.name != player]
    ...
    def set_queue(self, start_pos):
        ...
        self.player_queue = [p for p in queue if p.active and not p.is_all_in]
    ...
    def action(f):
        def wrapped(table, player, *args, **kwargs):
            ...
            if table.state == SHOWDOWN and f.__name__ not in ["fold", "show_hand"]:
                player.show_hand()
                return

            amount = f(table, player, *args, **kwargs)

            if table.state != PREFLOP or f.__name__ != "bet" or amount > 10:
                player.last_move = f"{f.__name__}{f" {str(amount)}$" if amount else ""}"
            ...
            table.round_stats.add_move(f.__name__, amount)
            ...
            if (len(table.player_queue) == 0):
                ...
                while len(table.player_queue) == 1:
                    table.start_round()
    ...
    def pre_flop(self): 
        ...
        self.set_queue(self.dealer_pos+3 % len(self.players))
    ...
    def rake(self):
        if len([p for p in self.players if p.active]) == 1:
            self.winning_player = [p for p in self.players if p.active][0]
    ...
    def auto_play(self):
        if len([p for p in self.players if p.active]) == 1:
            self.state = RAKE
            self.winning_player = [p for p in self.players if p.active][0]
            self.winning_hand = (0, 0)
            return False

        if self.player_queue and self.player_queue[0].is_computer:
            self.player_queue[0].play()
            return True
    ...
    def setup(self):
        ...
        for idx, player in enumerate(self.players):
            player.position = (idx - self.dealer_pos) % len(self.players)
    ...
    def toJSON(self, player_name=None):
        return {
            ...
            'players': [p.toJSON(player_name, self.show_cards) for p in self.players],
            'player_queue': [p.toJSON(player_name, self.show_cards) for p in self.player_queue],
            ...
            'winning_hand': self.winning_hand and [self.winning_hand[0], [card.shortName for card in self.winning_hand[1]]]
        }
...
class Player():
        ...
        def call(self):
            ...
            if (self.current_bet == self.table.required_bet):
                self.table.check(self)
            elif self.can_pay(self.table.required_bet): 
        ...
        def toJSON(self, player_name, show_cards=EVERYONE):
            ...
            if self.name == player_name or "show_hand" in self.last_move or (show_cards == ONLY_BOTS and self.is_computer) or show_cards == EVERYONE:
