...
parser = argparse.ArgumentParser(
    prog='Template bot',
    description='A Turing Games poker bot that always checks or calls, no matter what the target ra is (it never folds and it never raises)')
...
class Loki(Bot):
    ...
    def act(self, state: types.PokerSharedState, hand: Tuple[types.Card, types.Card]) -> types.Action:
        ...
        me = [p for p in state.players if p.id == "l3"][0]


        print(state.target_bet)
        raise_amount = state.target_bet - me.current_bet

        print(ehs)

        if ehs >= raise_amount * 2 / (state.pot + raise_amount * 2):
            return self.make2(state)

        elif ehs >= raise_amount / (state.pot + raise_amount):
    ...
    def make2(self, state: types.PokerSharedState):
        ...
        print("(make2):", state.pot * 2 / 3, end=' ')
        return {'type': 'raise', 'amount': state.pot * 2 / 3}
