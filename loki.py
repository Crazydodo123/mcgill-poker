...
class Loki(Bot):
    ...
    def make1(self, state):
        ...
        # print("(make1):", end=' ')
        # if self.table.round_stats["bet"] != 0:
        #     return {'type': 'fold'}
        # else:
            return {'type': 'call'}

    def call2(self, state):
        '''Always check/call, whatever bet is on the table.'''
        print("(call2):", end=' ')
        return {'type': 'call'}

    def make2(self, state):
        '''Bet/raise if less than two bets/raises have been made this round, otherwise call.'''
        print("(make2):", end=' ')
        return {'type': 'bet', 'amount': state.target_bet}

    def make4(self, state):
